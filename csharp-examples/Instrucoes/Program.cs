namespace Instrucoes 
{
    class Program 
    {
        ...
        static async void InstrucaoWhile(string[] args)
        {
            int i = 0;
            while (i < args.Length)
            {
    ...
    static void InstrucaoContinue(string[] args)
    {
        for (int i = 0; i < args.Length; i++)
        {
            if (args[i].StartsWith("/")) // se o args[i] começar com /
            {
                continue;
            }
            Console.WriteLine(args[i]);            
...
// w.Dispose(); limpa o objeto w da memória